...
double Сount(double left_border, double right_border, double simpson_factor,
  ...
  int steps_count_0 = lround((right_border - left_border) / actual_step[0]);
...
VinyaikinaEMultidimIntegrSimpsonOMP::VinyaikinaEMultidimIntegrSimpsonOMP(const InType &in) : I_res_(0.0) {
...
#pragma omp parallel num_threads(num_threads) default(none) \
    shared(limits, simpson_factor, actual_step, delta, function, num_threads) reduction(+ : res)
  {
    ...
    res += Сount(left_border, right_border, simpson_factor, limits, actual_step, function);
  }
